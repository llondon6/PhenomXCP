...
# For all sims 
# for a in [a for a in A if ('q1a06t120dPm1' in a.simname) ]:
for a in A:
    ...
    # frame['init-j'] = frame['init-j'].__symmetrize__(xparity=True)
    ...
    for ll in range(2,lmax+1):
        ...
        subframe[ll]    = frame['init-j'].selectlm( [(ll,m_) for m_ in range(-ll,ll+1)] )
        ...
        alert('Saving diagnostic plot to "%s"'%yellow(png_file_path))
